...
| [RUSTSEC-2026-0217](https://rustsec.org/advisories/RUSTSEC-2026-0217) | `tract-nnef` 0.21.10 | medium | `tract-nnef` → `tract-onnx` → `provcheck-{watermark,audioseal,wavmark}` | Integer overflow in the NNEF `.dat` tensor parser yields an out-of-bounds read on model load. Companion to the `tract-onnx` external-data path-traversal advisory ([GHSA-h668-6x6g-f8r5](https://github.com/advisories/GHSA-h668-6x6g-f8r5) / CVE-2026-55832), which has no RustSec ID and so is Dependabot-only. **A fix exists** (tract 0.21.16 / 0.21.17), but it pulls a newer `tract-linalg` whose `time` requirement needs Rust 1.88, above our deliberate MSRV 1.85 (`time` is held at 0.3.49 to preserve it, see the `time` note). **Not reachable at runtime:** both are model-LOAD advisories, and our audio detectors compile ONNX weights in via `include_bytes!` and hand tract a `&[u8]` slice, so no untrusted model file is ever parsed (same path as the `memmap2` row above). Held pending an MSRV-1.88 decision; the row drops when tract leaves the lockfile (the ort migration) or when the fix lands under our MSRV. |
...
- **`serde_with`** ([GHSA-7gcf-g7xr-8hxj](https://github.com/advisories/GHSA-7gcf-g7xr-8hxj), medium) — a `KeyValueMap` serialization panic on empty sequence or map entries. Fixed in 3.21.0, which requires Rust 1.88, above our MSRV 1.85. **Not reachable: `KeyValueMap` is not used anywhere in the workspace.** Held; revisit if the fix lands under our MSRV or the type is ever used.
