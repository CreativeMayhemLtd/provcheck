...
//!
//! Usage:
//!   cargo run --release -p provcheck-audioseal --example audioseal_embed_roundtrip
//!
...
use provcheck_audioseal::{audio, encode, registry};
...
fn main() -> anyhow::Result<()> {
    ...
    println!("== audioseal_embed_roundtrip ==");
    ...
    let t1 = Instant::now();
    let marked = encode::embed(&waveform, brand_id, None)?;
    ...
    let out_path = std::env::temp_dir().join("provcheck-audioseal-marked.wav");
    write_wav(&out_path, &marked, audio::SAMPLE_RATE)?;
    println!("  wrote: {}", out_path.display());

    let t2 = Instant::now();
    let result = provcheck_audioseal::detect(&out_path)?;
    ...
    if matches!(
        result.brand,
        Some(provcheck_audioseal::WatermarkBrand::Doomscroll)
    ) {
        println!();
