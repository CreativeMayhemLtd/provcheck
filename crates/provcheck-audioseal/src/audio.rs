//! Audio decode + downmix-to-mono + resample to 16 kHz (AudioSeal's
//! training rate).
//!
//! Mirrors `provcheck-watermark::audio` for symphonia/rubato wiring —
//! the only structural difference is `SAMPLE_RATE = 16_000` here vs
//! `44_100` for silentcipher. Kept as a sibling module rather than a
//! shared crate so each detector family owns its target-rate
//! constant in the same code that consumes it.
...
/// Target sample rate for AudioSeal. Both the detector and the
/// generator are trained at 16 kHz; passing a different rate
/// silently degrades the watermark.
pub const SAMPLE_RATE: u32 = 16_000;

/// Default AAC LC encoder priming when the source container's
/// metadata does not surface it. See the matching constant in
/// `provcheck-watermark/src/audio.rs` for the rationale. Public
/// issue #24 (v0.5.2).
const AAC_DEFAULT_PRIMING_SAMPLES: u32 = 1024;

/// Encoder priming for the given track, falling back to
/// [`AAC_DEFAULT_PRIMING_SAMPLES`] when symphonia did not surface
/// the delay metadata and the codec is AAC.
...
/// Decode `path` to a mono f32 waveform at AudioSeal's target rate
/// (16 kHz). LAME priming + end padding are trimmed when the input
/// is an MP3 — same fix v0.3.3 shipped for the silentcipher path.
...
/// Mirrors `provcheck-watermark::audio::decode_to_stereo_44k1`.
...
/// Resample `src` from `src_rate` to `dst_rate`. Same rubato config
/// as provcheck-watermark.
