...
The table's ✓ marks are grounded in v0.5.2's empirical parity sweep and the ongoing pre-push AAC delivery smoke gate ([`scripts/check-before-push.sh`](../scripts/check-before-push.sh) step 3). The silentcipher-fine-tune column marks are **planned targets** for a codec-robustness fine-tune that is not yet shipped; actual post-training numbers will be published when that work lands.
