...
|---|---|---|
| **v1.3.0** | 2026-08-28 | **Backfire 3.0: a keyed image mark that survives learned removal.** The Backfire mark (in `backfire/`, source-available under BUSL-1.1, free for non-commercial use or a commercial license, never bundled into the Apache-2.0 binary) is now optimized to be a fixed point of two AI regeneration attacks at once, a diffusion purifier and a learned neural codec, so both removers leave the keyed identifier readable. This is validated directly against the public tool behind the "invisible watermarks are provably removable" result (Zhao et al., arXiv:2306.01953): across a twenty-image COCO set the keyed serial survives the diffusion regeneration attack 20/20 (including on a purifier it was never tuned against), the neural-codec (VAE) attack 19 to 20/20, and benign JPEG and resize 20/20, all read back through the tool's own reader with zero false positives over 1,000 unmarked images. The honest limit, disclosed in `backfire/LIMITS.md`, is that Backfire does not survive controllable regeneration from clean noise (CtrlRegen) at real strength. Imperceptibility is 30 dB PSNR. The neural-codec objective is on by default (`--codec-eot`, `--codec-iter`); `compressai` joins the embed dependencies. `provcheck --backfire-read` and the desktop Backfire tab are unchanged. Repository housekeeping: internal-only operations tooling and notes are removed from the public tree. Workspace 1.2.0 to 1.3.0. |
| **v1.2.0** | 2026-08-19 | **A provcheck maintenance update, and the launch of Backfire.** Backfire (new, in `backfire/`, source-available under BUSL-1.1, free for non-commercial use, or a commercial license, and never bundled into the Apache-2.0 binary) is an imperceptible, keyed image watermark that AI provenance-stripping (diffusion regeneration) attacks amplify instead of remove: the mark is optimized to be a fixed point of the purifier, so running the stripper makes the keyed identifier read stronger. Validated at 512 px over 24 diverse COCO photos (survives a real diffusion purifier 24/24 and a held-out purifier 24/24, amplifies 21/24, 34 dB), with zero false positives over 1,000 unmarked images from a robust decoy-normalized decode. Ships its own `LIMITS.md` and self-red-team: an aggressive band-notch strips the linear mark, answered by a tamper tripwire (0 strip-and-evade gaps across an 81-config grid), and it is not crop-invariant. Integrated as an experimental option: `provcheck --backfire-read` on the CLI, and an experimental Backfire verify tab in the desktop app, both shelling out to the standalone tool and requiring the user's key. `provcheck-mellin` (the opt-in Fourier-Mellin audio channel) adopts the same BUSL-1.1 (free non-commercial) or commercial model as Backfire, and is wired in the same way: `provcheck --mellin-read` on the CLI and a Keyed marks tab in the app (Backfire and Mellin side by side) shell out to the standalone `provcheck-mellin` binary over a machine-readable `--json` protocol, with the seller secret travelling only as a file path and a pre-push gate refusing any linkage of BUSL code into the Apache binary. Both keyed tools carry one shared keyed-forensic-tools EULA and TOS, a per-version BUSL Change Date of four years from first public distribution, and AGPL-3.0-or-later as the Change License. The installer bundles curated, C2PA-signed rAIdio.bot sample audio plus keyed marked/unmarked test pairs and a walkthrough README. The desktop app additionally gains a per-family "Scan for" selector on the Watermark tab and a seventeen-finding UX-audit batch: per-tab file dialogs with an all-files fallback, real in-place signing, in-app confirmations replacing unreliable native dialogs, frontend diagnostics into provcheck.log with spinner watchdogs, and error states that render as errors. MSRV rises to 1.88; the serde_with, time, event-listener, and spin advisories clear, and the remaining tolerated set is re-documented with current reasoning in SECURITY.md. Core maintenance: image verification degrades to a clean error instead of hard-crashing on a missing or wrong ONNX Runtime, plus dependency, advisory, and CI-gate hardening. Workspace 1.1.11 to 1.2.0. |
| **v1.1.6** | 2026-07-01 | **Codec-robust silentcipher fine-tune — full plan + training scaffold on disk. Task #144 body moves from "blocked on external GPU collaborator" to "5090 available; scaffold ready to fire on two 6-hour nights." No training executed yet; no cargo cycles, no push.** Three new operator-facing docs: (a) `docs/audio-watermark-survival-range.md` — the honest per-attack per-family survival table for silentcipher / AudioSeal / WavMark including sample-rate / bit-depth / channel-layout range and the "what nobody in 2026 has" reality on full-DJ-remix survival, (b) `docs/silentcipher-codec-robust-fine-tune.md` — the formal training plan (target metrics, methodology, 2×6h schedule, bulletproofing, deliverable structure, risk table, integration plumbing), (c) `docs/multi-family-embed-workflow.md` — the operator-side pattern for stacking all three families to get the union of survival envelopes. New training scaffold at `tools/train-silentcipher-codec-robust/` (outside the Rust workspace, doesn't affect release matrix or CI): `README.md`, `requirements.txt` (torch 2.4+ for Blackwell CUDA 12.4, silentcipher, encodec, librosa, soundfile, huggingface_hub), `preflight.py` (7 environment checks, RED/AMBER/GREEN banner, exits 1 on RED), `configs/night1.yaml` + `configs/night2.yaml` (single source of truth for hyperparameters — no argparse overrides so exact config for any checkpoint is reconstructible), `scripts/night_run.sh` (orchestrator: preflight → tmux → wait → validate), `scripts/safe_stop.sh` (touches `stop.flag` for graceful stop; never `kill -9`). `train.py`, `validate.py`, `export_onnx.py`, and `codec_augmentation/` land in follow-up iteration commits once operator confirms Encodec Option A vs psychoacoustic Option B for the augmentation path. All doc + Python; no cargo dependency, no CI. [skip ci] |
| **v1.1.5** | 2026-07-01 | **Backlog cleanup pass — three sub-decisions folded into doc-comments + memory drawers.** kit lock/unlock clap doc-comments; kit reconcile decision (NOT building — manual workflow documented); PKCS#12 re-check trigger; test coverage survey (~1010 tests across 17 crates). Full detail in an internal note memory + docs. |
| **v1.1.4** | 2026-07-01 | **Globe icon on the top-bar `provcheck.ai` link.** Inline SVG (14×14, current-color stroke) inside the `#source-link` anchor to visually signal the external-link nature (Tauri's IPC-based `open_url` fires; the icon is the operator affordance). Styling: `.attribution a` becomes `inline-flex` with a 5px gap; `.link-globe` sits at 0.75 opacity and lifts to 1 on hover. Creative Mayhem UG link unchanged (it identifies the entity, not a domain). |
| **v1.1.3** | 2026-07-01 | **Tauri window title update: `provcheck — C2PA verifier` → `Provcheck.AI: Digital Media Provenance Tool`.** The old title was C2PA-only; the app now surfaces six watermark detector families across the Watermark tab, DID-anchored attestation via atproto, and the creator-side Sign + Keys tabs — the "C2PA verifier" framing no longer captures scope. One-line edit in `app/src-tauri/tauri.conf.json` `app.windows[0].title`. Windows-taskbar / macOS-app-switcher / Linux-window-manager labels pick up the new string on next build. |
| **v1.1.2** | 2026-07-01 | **Release-tag gate grows a third refusal: no FC-declaration file → no `v*.*.0` push. Iteration counter documented as uncapped. Docs + memory + gate script all say the same thing.** New refusal rule (3) in `scripts/check-before-push.sh`: every `v*.*.0` tag push requires `docs/release-fc/<tag>.md` to exist AND be committed on the branch being pushed (`git ls-tree HEAD` check). File is content-agnostic — its presence is the operator's affirmative FC declaration. `docs/semver-policy.md` gains an "iteration counter has no cap; no automatic promotion" section explicit that Z in `vX.Y.Z` goes `1, 2, ..., 9, 10, ..., 99, 100, ..., 999, 1000, ..., 10⁶⁵` if that is what iteration pace demands. No rollover. No auto-promotion. `docs/release-process.md` pre-flight checklist grows step 0: "FC declaration file exists and is committed" with a minimum-viable file shape. Memory index entry rewritten to reflect the uncapped iteration + FC-only-promotion rule. This locks the iteration-counter policy into the gate, docs, and process so it stays consistent. |
| **v1.1.1** | 2026-07-01 | **`scripts/check-before-push.sh` grows a hard release-tag gate that REFUSES `v*.*.0` pushes violating two rules. Also folds in the v1.1.0 tab redesign that was queued unpushed.** The gate reads the ref list from the pre-push hook's stdin and: (1) **Cadence rule** — refuses ANY push of a `v*.*.0` tag if another `v*.*.0` tag exists on the local repo with a `creatordate` within the last 24 hours. Prints the offending tag name + "hours ago" + the rule it's enforcing (max one v* per 24h, to limit release-matrix CI spend). Exit 1 aborts the push. (2) **Force-rewrite rule** — refuses any push of a `v*.*.0` tag when `remote_sha != 0000...` (meaning the tag already exists on origin at a different SHA). Prints the memory rule prohibiting this (an internal note: "Force-rewriting a `vX.Y.0` tag that already fired the matrix is even worse than the double-tag"). Emergency override for both rules is `git push --no-verify`, with the convention that any bypass gets noted in the commit body / tag annotation for the audit trail. Gate skips itself when stdin is a tty (CLI invocation of the script for manual runs, not the git pre-push path). **Gate origin**: a heavy release cycle motivated a gate that cannot be skipped in the build process. **Also included**: the v1.1.0 Tauri tab redesign — dedicated `Watermark` and `Detect` tabs; `open_url` Tauri command wiring for the top-bar external links (`provcheck.ai` / `Creative Mayhem UG`) that were silent no-ops on v1.0.0 because Tauri 2 sandboxes `target="_blank"`. Workspace `0.9.88 → 1.1.0` (from queued v1.1.0 draft) → **1.1.1** (this iteration). |
| **v1.1.0** | 2026-07-01 | **[SUPERSEDED by v1.1.1 — v1.1.0 tag on dev points at an inline-design commit that predates the Watermark + Detect tab redesign the user asked for. Left in place per the iteration-tags-for-fixes rule: do not force-rewrite an existing v*.*.0 tag; iterate as v1.1.Z and let the next real v*.x.0 supersede naturally.]** Tauri app gains Watermark + Detect tabs. All six detector families accessible as first-class surfaces. External branding links open in browser (were silent no-ops on v1.0.0). Two dedicated tabs join the existing Verify / Keys / Sign trio. **Watermark tab** (`ProvcheckStamp`-independent): drop any file → runs all six shipped detector families (silentcipher / AudioSeal / WavMark on audio, TrustMark-B on image, per-frame TrustMark + temporal vote on video, SynthID-text on text) → renders per-detector badges. Backed by the new `watermark_only` Tauri command which runs the shared `push_all_watermarks` helper (factored out of `verify_file`'s watermark loop so both tabs share the identical dispatch code). **Detect tab** (AI-content detection surface): shows an educational empty-state describing the two routes to activate it — (Route A) Creative Mayhem paid-DLC pack on the v1.x roadmap, (Route B) operator-supplied Apache-2.0 wrapper crate that implements `provcheck_detect::Detector` and registers itself via `DetectorRegistry`. Backed by the `detect_only` Tauri command; FOSS core registers zero detectors so it returns an empty `detections` list, matching the tab's empty state. **External-URL opener**: v1.0.0's top-bar `provcheck.ai` + `Creative Mayhem UG` anchor tags with `target="_blank"` were silent no-ops because Tauri 2 sandboxes the webview. v1.1.0 adds the `open` crate + an `open_url` Tauri command that shells out to the platform URL handler (Windows: `cmd /c start`, macOS: `open`, Linux: `xdg-open`); the frontend delegates on `document.body` and IPC's every `http(s)`-prefixed anchor click through it, refusing non-`http(s)` schemes on the backend as defence against a compromised frontend shelling `file:///`. **Verify tab** still runs the six-detector dispatch inline when the "Run watermark detection" checkbox is on (backward-compat), and now shows a "No watermarks detected in any of the six shipped families…" empty note instead of hiding the badge slot entirely (v1.0.0 silent-hide made a drop-a-PNG-see-nothing case read as if detection never ran). Workspace bumped 0.9.88 → 1.1.0 (minor: additive tabs + external opener + shared helper factor, no breaking change to any command contract). |
| **v0.9.88** | 2026-07-01 | **`.github/workflows/release.yml` wires SSL.com eSigner signing on the Windows arms of both build (CLI) and build-gui (Tauri) jobs — v1.0.0 fires signed IF the four SSL_COM_* Secrets are set on the dev repo; skips gracefully if not.** Both jobs' Windows steps: (a) if `SSL_COM_TOTP_SECRET` env is empty → log a skipping note, exit 0 (matrix continues, ships unsigned). (b) if set → download SSL.com CodeSignTool zip, expand under `$USERPROFILE`, materialise a per-run `signing.json` in `$RUNNER_TEMP` from the four secrets, call `scripts/sign_release.ps1` per-file, delete the materialised `signing.json` before step end (never persists past the runner's `RUNNER_TEMP` scope). CLI job signs `provcheck.exe` + `provcheck-kit.exe` between build and archive-staging. GUI job signs the inner `provcheck-app.exe` first, then every installer under `app/src-tauri/target/release/bundle/**/*.{exe,msi}` (NSIS setup + MSI) between `tauri build` and `Stage bundles`. Updated the build-gui header comment: was "Bundles are UNSIGNED for now — Signing is a follow-up once certs exist," now describes the Secrets-gated posture with the SSL.com eSigner cert. |
| **v0.9.87** | 2026-07-01 | **WATERMARK_LICENSE_POLICY.md woefully-out-of-date sweep + tests/fixtures anchor drop + SBOM regeneration for public mirror parity.** (1) `WATERMARK_LICENSE_POLICY.md` had a paragraph claiming "AudioSeal and WavMark are scaffold-only ... `implementation pending` stub `detect()`" and "SynthID stays out on license grounds" — both flatly wrong at v0.9.x. AudioSeal + WavMark integrated since v0.4/v0.5; SynthID-text integrated in v0.7 phase 7e (statistical detector, no model weights so no license question). Replaced with an accurate six-family status summary. Added Video and Text family tables mirroring the Audio and Image tables. Corrected the TrustMark image row's "lands at 7b-inference" future-tense to "Integrated ... v0.7 phase 7b + 7b-followup migrated tract → ort." (2) `crates/provcheck/tests/fixtures/README.md` said "As of v0.9.64 the suite has 13 tests" — count is still 13 but the version anchor is 23 iterations stale. Dropped the anchor. (3) Regenerated CycloneDX + SPDX SBOMs at v0.9.87 tag (provcheck: 469 components, provcheck-kit: 646 components, all workspace crates at 0.9.87, SHA-256 sidecars present) so hash-verify against a current dependency graph works today. SBOMs live under `target/sbom/` (gitignored — release matrix regenerates them fresh per `v*.*.0` tag). |
| **v0.9.86** | 2026-07-01 | **Code-signing scaffold lands — SSL.com eSigner cloud path wired locally, ready for v1.0.0 CI activation.** New file `scripts/sign_release.ps1` (~150 lines, PowerShell): Authenticode signer for Windows binaries via SSL.com's `CodeSignTool`. Reads config from a `signing.json` at repo root (GITIGNORED), pulls SSL.com credentials from either (a) Windows Credential Manager under a named target (local dev machine, no plaintext on disk) OR (b) inline `username` + `password` fields (CI runners materialising from GitHub Secrets). CodeSignTool derives the per-sign OTP from SSL.com's base64 `secret_code`; sign_release.ps1 hard-fails if the credential is missing rather than silently trying a fallback. Signed → optional signtool verify → audit log append. New file `signing.json.example` at repo root documents the config shape with in-file comments. `.gitignore` adds `signing.json` (config with secrets) + `provcheck-signing.log` (default audit log path). `docs/release-process.md` adds a full "Code-signing (Windows binaries)" section: what gets signed (`provcheck.exe`, `provcheck-kit.exe`, `provcheck-app.exe`, NSIS setup, MSI), local + CI procedures, the GitHub Secrets the CI wiring needs, and verification via `Get-AuthenticodeSignature`. `SECURITY.md` "Scope" section gets a "Windows binary integrity" bullet describing the SSL.com OV cert chain (Creative Mayhem UG → SSL.com Code Signing Intermediate CA RSA R1), RFC-3161 timestamping so signatures survive cert expiry, and the release-line-tags-only constraint (iteration tags aren't signed). **NOT wired yet:** `.github/workflows/release.yml` still produces unsigned artefacts; the CI activation lands in v1.0.0 once the signing secrets are configured in CI. Doc + script commit; no CI-behaviour change until v1.0.0. |
