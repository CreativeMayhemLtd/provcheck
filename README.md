...
| **v1.1.2** | 2026-07-01 | **Release-tag gate grows a third refusal: no FC-declaration file → no `v*.*.0` push. Iteration counter documented as uncapped. Docs + memory + gate script all say the same thing.** New refusal rule (3) in `scripts/check-before-push.sh`: every `v*.*.0` tag push requires `docs/release-fc/<tag>.md` to exist AND be committed on the branch being pushed (`git ls-tree HEAD` check). File is content-agnostic — its presence is the operator's affirmative FC declaration. `docs/semver-policy.md` gains an "iteration counter has no cap; no automatic promotion" section explicit that Z in `vX.Y.Z` goes `1, 2, ..., 9, 10, ..., 99, 100, ..., 999, 1000, ..., 10⁶⁵` if that is what iteration pace demands. No rollover. No auto-promotion. `docs/release-process.md` pre-flight checklist grows step 0: "FC declaration file exists and is committed" with a minimum-viable file shape. Memory index entry rewritten to reflect the uncapped iteration + FC-only-promotion rule. This exists because iteration counter overflow-based promotion was a mental model the operator has NEVER endorsed and the assistant has repeatedly rationalized toward — the gate + docs + memory triad forces the rule to survive across sessions. |
| **v1.1.1** | 2026-07-01 | **`scripts/check-before-push.sh` grows a hard release-tag gate that REFUSES `v*.*.0` pushes violating two rules. Also folds in the v1.1.0 tab redesign that was queued unpushed.** The gate reads the ref list from the pre-push hook's stdin and: (1) **Cadence rule** — refuses ANY push of a `v*.*.0` tag if another `v*.*.0` tag exists on the local repo with a `creatordate` within the last 24 hours. Prints the offending tag name + "hours ago" + the memory rule it's enforcing (`feedback_release_cadence_budget.md` — max one v* per 24h, historical burn was ~€25/week of Actions minutes across the Tauri + 10× macOS matrix). Exit 1 aborts the push. (2) **Force-rewrite rule** — refuses any push of a `v*.*.0` tag when `remote_sha != 0000...` (meaning the tag already exists on origin at a different SHA). Prints the memory rule prohibiting this (`feedback_iteration_tags_for_fixes.md`: "Force-rewriting a `vX.Y.0` tag that already fired the matrix is even worse than the double-tag"). Emergency override for both rules is `git push --no-verify`, with the convention that any bypass gets noted in the commit body / tag annotation for the audit trail. Gate skips itself when stdin is a tty (CLI invocation of the script for manual runs, not the git pre-push path). **Gate origin**: the 2026-07-01 recovery cycle burned 6+ full release-matrix runs in one day (`v0.8.0` → `v0.9.0` → `v1.0.0` → three `workflow_dispatch` signing tests → `v1.1.0` inline-design mistake); the user's demand was "put this in some sort of gate that you can't forget in your build process." **Also included**: the v1.1.0 Tauri tab redesign — dedicated `Watermark` and `Detect` tabs; `open_url` Tauri command wiring for the top-bar external links (`provcheck.ai` / `Creative Mayhem UG`) that were silent no-ops on v1.0.0 because Tauri 2 sandboxes `target="_blank"`. Workspace `0.9.88 → 1.1.0` (from queued v1.1.0 draft) → **1.1.1** (this iteration). |
