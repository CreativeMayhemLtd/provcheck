...
|---|---|---|
| **v1.1.0** | 2026-07-01 | **Tauri app now wires all six watermark detector families the workspace ships. v1.0.0 GUI only called `provcheck_watermark::detect` (silentcipher) + `provcheck_audioseal::detect` + `provcheck_wavmark::detect` — the three AUDIO detectors. Drop a PNG / MP4 / .txt into the app and detection ran silently against nothing.** v1.1.0 adds `provcheck-image` (TrustMark image, BCH-5 ecosystem interop), `provcheck-video` (per-frame TrustMark + temporal majority-vote), and `provcheck-synthid-text` (Bayesian tournament-sampling z-score) to `app/src-tauri`'s deps, wires them into `verify_file`'s dispatch chain alongside the three audio detectors. Every dropped file now goes through all six families; each detector's own `detect(&path)` gates its applicability (audio detectors return `Err` on a PNG, etc.). **Frontend UX fix:** `renderWatermarks` in `app/dist/app.js` previously hid the watermark panel entirely when the returned list was empty — even when the user had opted IN to detection via the "Run watermark detection" checkbox — which made a drop-a-PNG-see-nothing case indistinguishable from "detection didn't run." Fix: when the checkbox is on AND the result list is empty, show a `"No watermarks detected in any of the six shipped families (silentcipher, AudioSeal, WavMark, TrustMark image, TrustMark video, SynthID-text)"` note in the same slot with dashed-border empty styling (`.watermarks-empty`). When the checkbox is off, still hides (user opted out → silence expected). Workspace bumped 0.9.88 → 1.1.0 (minor: additive feature, no breaking change). Note: AI detect (`provcheck-detect` trait dispatch via `--detect ai`) is not wired into the GUI yet — the FOSS core registers zero detectors, so a GUI toggle would look interactive but do nothing; waits for the paid-DLC detector pack OR an operator-supplied open-source detector before it earns a place in the UI. |
