...
Issues and pull requests are welcome. File bugs and feature requests
on this repository's issue tracker. For a change, fork the repository,
branch off `main`, and open a pull request against `main`. Build and
test with `cargo build --workspace` and `cargo test --workspace`. See
[CONTRIBUTING.md](./CONTRIBUTING.md) for the full guide (building,
house style, and the dependency-advisory policy), and
[SECURITY.md](./SECURITY.md) to report a vulnerability privately.

The intended design is: `provcheck` (core
