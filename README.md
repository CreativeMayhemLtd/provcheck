...
modalities (scaffolds; per-frame TrustMark + temporal vote
and SynthID-text logit-bias detection wire in v0.7.x).
...
|---|---|---|
| **v0.9.0** | 2026-06-29 | **Video + text modalities wired; ComfyUI node live; CUDA EP diagnostic.** Closes the 7d / 7e wiring left as scaffolds in v0.7.0 and lands the ecosystem-reach piece from the v0.9 staging release. **Video (7d):** `provcheck-video` now performs per-frame TrustMark-B inference via ffmpeg shell-out (one frame every 2 s, capped at 30), then temporal majority-vote on the recovered brand ids. Status=Detected when ≥3 frames recover the same brand id; Degraded when at least one frame recovers any brand id below threshold; NotDetected otherwise. ffmpeg-on-PATH check fails gracefully with an install hint. **SynthID-text (7e):** `provcheck-synthid-text` ships the real tournament-sampling Bayesian detection — for each token at position i, compute `g_i = hash(text[i-W..i], text[i], salt) ∈ [0, 1]`; aggregate `mean_g`; compute z-score against the unwatermarked baseline `0.5 ± sqrt(1/(12N))`; map to confidence via the standard normal CDF (Abramowitz-Stegun erf approximation). Pure-Rust SHA256-based hash, no FFI. Default word-level tokenizer ships now; HF subword tokenizer integration for Gemma / Llama / Qwen accuracy lands in v0.9.x. Unit tests cover both baseline (unmarked text scores near 0.5) and constructed-watermarked round-trip (mean_g > 0.7, z > 3, confidence ≥ 0.95). **ComfyUI node (9c):** `python/comfyui-node/` now actually calls `provcheck-kit stamp` per generation. Saves tensor → temp PNG, shells out, reads stamped PNG back. Fails closed (passes input through with console warning) if `provcheck-kit` is missing from PATH — render queues don't crash. Brand-agnostic; any creator with their own atproto identity uses the same node. **CUDA EP diagnostic (issue #32):** `--features cuda` binary now uses `.error_on_failure()` to detect when ort silently downgraded to CPU, emits a clear WARNING naming the prereqs, falls back explicitly to CPU. Default builds unaffected. |
